...
df_ori = pd.read_csv('1_origin/NHANES_2009_2010_Merged_Clean_Data_added_TG.csv')
...
X_cat = df_ori[categorical_feature_columns].astype(int)
X_cat['PreDM'] = X_cat['Diabetes'].apply(lambda x: 'B' if x==2 else 'A')
X_cat['DM'] = X_cat['Diabetes'].apply(lambda x: 'B' if x==1 else 'A')
X_cat['DM_PreDM'] = X_cat['Diabetes'].apply(lambda x: 'A' if x==0 else 'B')
X_cat = X_cat.drop('Diabetes',axis=1).replace({0: 'A', 1: 'B'})
X_cat = X_cat.replace({'A': 0, 'B': 1})
...
df_km = pd.read_csv('1_origin/km_event_year.csv', index_col=0)
